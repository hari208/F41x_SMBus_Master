...
void Delay(unsigned val){ int i=0;
  for( i=0; i<=val; i++);}
...
// TEST CODE-------------------------------------------------------------------
   ...
   while (1)
   {
	 ...
	 TM1640_init(brightness_100_pc);    Delay(0x55);
	 TM1640_send_command(0x88 | 0x07);  Delay(0x55);
     TM1640_send_data(1, 0x49);			Delay(0x55);
     TM1640_send_data(2, 0x58);			Delay(0x55);
	}
...
{                                                       
      ...
      SMB_DATA_OUT = value;              // Define next outgoing byte
